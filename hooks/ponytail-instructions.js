...
function getFallbackInstructions(mode) {
  return 'PONYTAIL MODE ACTIVE — level: ' + mode + '\n\n' +
    ...
    'Bug fix = root cause, not symptom: grep every caller and non-call entry path (callbacks, retries, reload/restore, attach, redirects, persisted state, concurrent calls), then fix the shared function once; patching only the named path leaves sibling paths broken. Inputs have siblings too: run the named bad shape\'s neighbors (shorter, empty, zero) through the fixed line before shipping — each must produce the correct value or an explicit rejection, not a silently wrong result.\n\n' +
    '## Before shipping\n\n' +
    ...
    'bound external time, bytes, items, retries, memory, path lengths, and name collisions (cap or evict state keyed by client-controlled values; cleanup that only runs when the same key returns never shrinks the map); preserve required request semantics while reapplying security policy; ' +
    ...
    'Lazy code without its check is unfinished: non-trivial logic leaves ONE risk-targeted runnable check behind for the riskiest boundary, cancellation, partial failure, replay/round-trip, or explicit false/zero/empty state (assert-based demo/self-check or one small test file with a single test function; one check means one, no frameworks). Trivial one-liners need no test. ' +
