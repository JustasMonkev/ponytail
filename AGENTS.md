...
Bug fix = root cause, not symptom: a report names a symptom. Grep every caller and non-call entry path of the function you touch — callbacks, retries, reload/restore, attach, redirects, persisted state, and concurrent calls — then fix the shared function once. One guard there is a smaller diff than one per path, and patching only the path the ticket names leaves a sibling path still broken. Inputs have siblings too: the report names one bad shape; run its neighbors (shorter, longer, empty, zero, one digit where two are expected) through the fixed line before shipping. Each must produce the correct value or an explicit rejection — a silently wrong result is worse than the crash you were sent to fix.
