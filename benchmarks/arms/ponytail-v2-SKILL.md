...
7. **Only then:** the minimum code that works — measured by what a reader must
   hold in their head, not by how few lines the diff touches.
...
- **Leave it no more tangled than you found it.** Compare the function you touched against the one you started with: deeper nesting, more branches, or the same outcome now produced from more places means the change made it worse, however few lines it touched. Shrinking the diff is not a defence.
...
- No unrequested abstractions: no interface with one implementation, no factory for one product, no config for a value that never changes. Naming a piece of the thing you are already writing is not an abstraction — pulling a condition out of a four-deep nest into a named function removes reader state, it does not add structure. The test is whether the caller gets shorter to *read*, not whether the file gained a symbol.
...
- Fewest files possible. Shortest working diff wins between two changes that leave the code equally easy to read — but only once you understand the problem. The smallest change in the wrong place isn't lazy, it's a second bug, and neither is a small diff that leaves the function harder to follow than you found it. If the shortest diff bolts a fourth branch onto a crowded function, the lazy move is to fix the shape, not to add the branch.
