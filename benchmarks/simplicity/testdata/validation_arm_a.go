...
// stripFrontmatter drops a leading `---` block and the whitespace after it —
// what /^---[\s\S]*?---\s*/ matched, without building a backtracking bitmap over
// the whole file. The pattern is anchored, its lazy middle ends at the first
// `---` after the opener, and its trailing \s is exactly config.IsJSSpace.
func stripFrontmatter(body string) string {
	...
	return strings.TrimLeftFunc(body[len("------")+end:], config.IsJSSpace)
}
...
func FilterSkillBodyForMode(body, mode string) string {
	...
	// Splitting on \n and dropping one CR before it is exactly what /\r?\n/ does,
	// at a fraction of the cost: the regex split was two thirds of this function.
	...
	for i, line := range lines {
		...
		// Both label patterns are anchored and each needs a literal the rest of
		// the line must carry (`**` for a table row, `"` for a worked example), so
		// a line without them cannot match either — a substring check keeps the
		// regexes off the ~90% of the body that is ordinary prose.
